...
def add_data(
    ...
    """
    Add new data sources to a Vertex AI RAG corpus from GCS paths or URLs.

    Args:
        corpus_name (str): The name of the corpus to add data to. If empty, the current corpus will be used.
        paths (List[str]): List of GCS paths or URLs to add to the corpus.
                          Supported formats:
                          - Google Cloud Storage: "gs://{BUCKET}/{PATH}"
                          - Google Drive: "https://drive.google.com/file/d/{FILE_ID}/view"
                          - Google Docs/Sheets/Slides: "https://docs.google.com/{type}/d/{FILE_ID}/..."
                          Example: ["gs://my_bucket/ontology_overview.md", "https://drive.google.com/file/d/123"]
    ...
    if not paths or not all(isinstance(path, str) for path in paths):
        return {
            "status": "error",
            "message": "Invalid paths: Please provide a list of GCS paths or URLs",
            "corpus_name": corpus_name,
            "paths": paths,
        }
    ...
    if not validated_paths:
        return {
            "status": "error",
            "message": "No valid paths provided. Please provide GCS paths (gs://...) or Google Drive/Docs URLs.",
            "corpus_name": corpus_name,
